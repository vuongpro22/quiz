...
def parse_question(block: str, fallback_q_num: int) -> tuple[int, str, list[tuple[str, str]], int]:
    # NOTE: `Question:\s*(\d+)\s*(.*)` cannot span lines — in Python `.` does not match
    # newlines, so multi-line stems (lists before "(Choose N answer)") were dropped.
    ...
    options = []
    for key, text in option_pattern.findall(options_block):
